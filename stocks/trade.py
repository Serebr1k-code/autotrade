#!/usr/bin/env python3
"""Headless daily trader — no TUI, no prompts. Run via cron."""
import os, sys, requests, json, time, numpy as np, pandas as pd, warnings, joblib
warnings.filterwarnings('ignore')
from pathlib import Path; import pickle; import yfinance as yf
from datetime import datetime, timezone, timedelta
from tqdm import tqdm; import urllib3; urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)

API_KEY = os.environ.get("STOCK_API_KEY", "st_sim_live_XHjCjmIz_VBfr8WY4wo59N4uG_DMbh9QgRBS0zpmpsw")
...
scaler = d["scaler"]
pca = d.get("pca", None)
top_idx = d.get("top_idx", None)
cols = d.get("cols", [])
lgbm_n_in = len(top_idx) if top_idx is not None else 1

cal_coef, cal_int = 0.0, 0.0
...
# ── Load cache ──
cache = {}
for cp in [Path("cache_data.joblib"), Path("cache_data.backup")]:
    if cp.exists():
        try:
            cache = joblib.load(cp)
            break
        except:
            try:
                with open(cp, "rb") as f: cache = pickle.load(f)
                break
            except: pass
if not cache:
    print("No cache found"); sys.exit(1)
cache = {t:d for t,d in cache.items() if len(d) >= 200}
TICKERS = sorted(cache.keys())[:60]  # top 60 by market cap conviction
    ...
    if r.status_code==200:
        ...
        if expert_map:
            ...
            for d in pd.date_range(sorted(expert_map.keys())[0],pd.Timestamp('2026-12-31'),freq='D'):
                ...
                if d.date() in expert_map: lv=expert_map[d.date()]
except: pass

# ── Build features for each ticker ──
def build_one(t):
    d=cache[t].copy().rename(columns={"open":"Open","high":"High","low":"Low","close":"Close","volume":"Volume"})
    d.index=pd.to_datetime(d.index).tz_localize("America/New_York"); d.index.name=None
    ft,_=add_stationary_features(d)
...
# ── Predict ──
print(f"\nPredicting {len(TICKERS)} tickers...", flush=True)
predictions = []
for t in tqdm(TICKERS, desc="Predict"):
    try:
        ...
        cal = 1/(1+np.exp(-(cal_coef*raw+cal_int)))
...
if not selected:
    print("No signals with prices."); sys.exit(0)
...
if r and r.status_code == 200:
    ...
    print(f"  Cash before sell: ${cash:.2f}", flush=True)
    ...
    if sell_orders:
        print(f"  Selling {len(sell_orders)} positions...", flush=True)
        try:
...
accs = np.array([p[1] for p in selected])
weights = accs ** 2
weights /= weights.sum()
buy_orders = []
...
try:
    ...
    if r and r.status_code == 200:
        for x in r.json().get("results", []):
            ...
            st = x.get("status","")
            pr = x.get("price") or 0
            q = x.get("quantity") or 0
            e = x.get("error_code","")
            m = x.get("message","")
            if e or m: print(f"  {s}: {e} — {m}", flush=True)
            else: print(f"  ✅ {s}: BUY {int(q)}sh @ ${pr:.2f}", flush=True)
    else:
        print(f"  API error: {r.status_code if r else 'no response'}: {r.text[:200] if r else ''}", flush=True)
except Exception as e:
